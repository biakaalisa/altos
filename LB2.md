...
![image](https://github.com/user-attachments/assets/a28354ec-e21f-43c3-a375-f25c2f41c11d)
...
![image](https://github.com/user-attachments/assets/0cc9acf6-7424-480f-9faf-d8757bd47b72)

top:
- 01:44:25 — текущее время.
- up 37 min — система работает 37 минут с момента последнего запуска.
- 1 user — один пользователь в системе.
- load average — средняя загрузка за 1, 5 и 15 минут:
 -  0.14, 0.05, 0.03 — низкая нагрузка (очень хорошо, нет перегрузки).
 - Если эти числа близки к количеству ядер — система загружена.

tasks:
- 169 total — всего 169 процессов.
- 1 running — 1 активный процесс.
- 168 sleeping — процессы в состоянии ожидания (нормально).
- 0 zombie — нет "зомби"-процессов ( процесс, который завершился, но его родитель ещё не "убрал" его из памяти.)

CPU:
- us (1.1)	Пользовательские процессы (user) — приложения
- sy (1.4)	Системные процессы (system/kernel)
- ni (0.0)	Процессы с изменённым приоритетом (nice)
- id (97.5)	Свободный CPU (idle) — очень хорошо
- wa (0.0)	Ожидание ввода-вывода (disk IO wait)
- hi / si	Прерывания аппаратные/софт. — 0, значит нет проблем
- st (0.0)	Время, украденное гипервизором (в виртуалке). То время, в течение которого виртуальная машина (гость) хотела использовать CPU, но не смогла, потому что гипервизор (хост) отдал это время другим ВМ.

Меньшее значение — выше приоритет.
![image](https://github.com/user-attachments/assets/42c7a769-fbd4-40be-8362-a5aff4392838)

Для использования htop, предварительно его установили ```sudo apt install htop```

    htop

![image](https://github.com/user-attachments/assets/59171be8-04dd-4e09-840a-f56c2699efb8)

- Tasks: 113	Общее число процессов
- Load average: 0.48 0.25 0.09	Средняя нагрузка за 1/5/15 мин	
- Uptime: 00:57:13	Время работы системы	
- 1024M/1.92G	Использование RAM (1 ГБ из 2 ГБ)	
- 12.6M/975M	Использование swap (12.6 МБ из 975 МБ). Параметр Swap (подкачка) — это область на диске, куда система перемещает неиспользуемые данные из оперативной памяти (RAM), когда ей не хватает места

![image](https://github.com/user-attachments/assets/14cda607-5645-4153-8cce-bc265c5df474)

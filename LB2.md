...
В открывшийся файл добавили строку, которая отвечает за ежедневный бэкап файлов в 3.00:

      0 3 * * * /usr/bin/pg_dump -U postgres postgres > /backups/db_$(date +\%Y\%m\%d).sql && find /backups -name "*.sql" -mtime +7 -delete

- ```0 3 * * *```  - время, когда будет выполняться команда
- ```/usr/bin/pg_dump``` -U postgres mydb > /backups/db_$(date +\%Y\%m\%d).sql - непосредственно сама команда
   ...
   + ```> /backups/db_$(date +\%Y\%m\%d).sql``` - путь и название файла, в который бужет сохраняться копия базы
